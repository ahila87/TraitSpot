...
df = pd.DataFrame([r for r in mylines(file_path1, 0, 1000000)])
...
#data = df3.groupby('user_id').text.apply(lambda x: np.sort(x.values)[:-4:-1]).reset_index()

print(data)
    

data.to_csv('dataset6')
